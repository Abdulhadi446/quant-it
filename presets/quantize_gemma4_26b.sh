...
if [ -d "$MODEL_CACHE" ] && ls "$MODEL_CACHE"/models--google--gemma-4-26B-A4B 2>/dev/null | head -1 > /dev/null; then
    ...
    echo "  downloading google/gemma-4-26B-A4B (~50 GB)..."
...
$PYTHON quantize.py --preset gemma4_26b \
    ...
    --device cuda:0 \
    --expert-batch 8 \
    --output gemma-4-26b-A4B-ternary

# --- convert to GGUF ---
echo ""
echo "[5/5] converting to GGUF..."
$PYTHON quantize.py --gguf gemma-4-26b-A4B-ternary \
    --output gemma-4-26b-A4B-ternary.gguf
...
echo ""
echo "output:"
echo "  GGUF: gemma-4-26b-A4B-ternary.gguf"
echo ""
echo "use with llama.cpp:"
echo "  ./llama-server -m gemma-4-26b-A4B-ternary.gguf -c 4096"
echo ""
echo "use with ollama:"
echo "  ollama create gemma4-26b -f Modelfile"
echo "  # Modelfile: FROM ./gemma-4-26b-A4B-ternary.gguf"
