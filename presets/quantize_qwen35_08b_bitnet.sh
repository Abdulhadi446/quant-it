...
$PYTHON quantize.py --preset qwen35_08b \
    --mode 1bit \
    --device cuda:0 \
    --output "$WORK_DIR/Qwen3.5-0.8B-bitnet"

# --- convert to GGUF ---
echo ""
echo "[5/5] converting to GGUF..."
$PYTHON quantize.py --gguf "$WORK_DIR/Qwen3.5-0.8B-bitnet" \
    --output "$WORK_DIR/Qwen3.5-0.8B-bitnet.gguf"
...
echo ""
echo "output:"
echo "  GGUF: $WORK_DIR/Qwen3.5-0.8B-bitnet.gguf"
echo ""
echo "use with llama.cpp:"
echo "  ./llama-server -m $WORK_DIR/Qwen3.5-0.8B-bitnet.gguf -c 4096"
